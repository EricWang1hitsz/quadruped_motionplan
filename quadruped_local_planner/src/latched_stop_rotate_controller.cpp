...
bool LatchedStopRotateController::isPositionReached(LocalPlannerUtil* planner_util,
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
      quadruped_local_planner::getGoalPositionDistance(global_pose, goal_x, goal_y) <= xy_goal_tolerance) {
...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util,
  ...
  quadruped_local_planner::LocalPlannerLimits limits = planner_util->getCurrentLimits();
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    double angle = quadruped_local_planner::getGoalOrientationAngleDifference(global_pose, goal_th);
    //check to see if the goal orientation has been reached
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      //make sure that we're actually stopped before returning success
      if (quadruped_local_planner::stopped(base_odom, theta_stopped_vel, trans_stopped_vel)) {
        return true;
      }
    }
  }
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    if ( ! rotating_to_goal_ && !quadruped_local_planner::stopped(base_odom, limits.theta_stopped_vel, limits.trans_stopped_vel)) {
